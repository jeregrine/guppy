...
- **Generic list / variable-height list**: `list` adds GPUI `ListState`-backed variable-height rows with static/layout row children (`text`, `spacer`, and nested static `div`); stateful controls inside virtual rows and custom scrollbar parity remain partial.
...
| `examples/list_example.rs` | `Guppy.IR.list/2`, `<list />`, `Guppy.IR.uniform_list/2`, `<uniform_list />`, `examples/super_demo.exs` | partial | variable-height GPUI `ListState` rows support static/layout `text`/`spacer`/nested static `div` row IR; stateful controls inside virtual rows and custom scrollbar parity remain incomplete | ExUnit IR/template/native hidden-window coverage; Rust list-state/render helper tests; manual super_demo smoke |
