...
- **Custom painting / canvas / pattern painting**: first bounded `canvas` primitive supports ordered rect/rounded-rect drawing, slash-pattern rects, wrapper sizing/style, and coarse click events. Paths, text/image drawing, arbitrary custom elements, retained canvas resources, and per-command hit testing remain deferred in [`docs/future-primitives.md`](future-primitives.md#custom-painting-canvas-and-pattern-painting).
