...
| `examples/tree.rs` | nested `div`/`text` IR; simple repeated text rows via `uniform_list`; variable-height static/layout rows via `list` | partial | tree-specific interaction/virtualization is scoped separately in `docs/future-primitives.md`; only button/checkbox/radio row controls exist inside generic list rows | template/IR tests |
