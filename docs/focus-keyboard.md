...
- **list**: rows with click/context-menu callbacks are tab stops; Enter/Space activates row click callbacks and keyboard context-menu invocation emits row context-menu events. Row-local controls remain ordinary retained tab stops. Generic Up/Down row roving focus is not implemented yet.
