...
- [ ] Add data-table column resize/reorder and pinned header/column behavior only after navigation/focus semantics are stable. Done so far: data-table headers support keyboard `column_reorder` callbacks with source column, target neighbor, and direction so Elixir can own column order; pointer resize/reorder and pinned columns remain.
