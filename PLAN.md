...
- [x] Cache NIF load status in `Guppy.Native.Nif` after first success instead of calling `native_ping` through `apply` + rescue on every dispatch (`with_loaded/1`).
- [x] Remove the `bytes.to_vec()` copy on the IR decode worker handoff in `native/guppy_nif/src/lib.rs` if a borrow/ownership transfer is practical. Done: the decode job now owns the `OwnedBinary` (Send, process-independent); debug-native render latency was unchanged within noise.
...
- [x] Sweep the rest of `bridge_view/` for shared renderer plumbing: `roving.rs` now owns roving-focus navigation targets, neighbor computation, focus-handle prep, enter/space activation (held-repeat aware), and the shared list-row interaction surface; uniform-list, list, tree, and data-table renderers all consume it.
...
- [x] Remove the AI-slop tells from the examples: hero/title/subtitle headers restating the file name, self-describing info panels, rounded-xl card-in-card nesting, and shadows on static content are gone from the small and mid-size examples; `super_demo`/`style_gallery` remain deliberate showcases.
...
- [x] Align the mid-size examples to the shared language: canvas, data-table/tree, list-row-controls, menu, overlay, and multi-window demos restructured onto `Examples.UI`; kanban, priority3, and the app shell moved to the light palette with macOS-tag-style badges (their layouts kept, since they were functional toolbars rather than hero chrome); markdownview keeps its deliberate paper reading theme. Window titles now name the app, not the framework feature.
...
- [x] Verify each reworked example: all restyled examples passed headless template compile + IR validation; kanban, multi-window, and the four small examples also had brief live runs. README descriptions updated where scope changed. A human visual pass over the restyled mid-size examples is still worthwhile.
