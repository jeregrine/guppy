...
Box-spacing, size, position, display, visibility, overflow, cursor, border, radius, named color, shadow, flex/alignment, core text/font, and grid count/span helpers/classes already use canonical tuple ops: `Guppy.Style.py(1)` and template `class="py-1"` both produce `{:padding, :y, {:rem, 0.25}}`; `Guppy.Style.mx(:auto)` produces `{:margin, :x, :auto}`; `Guppy.Style.gap_x("px")` produces `{:gap, :x, {:px, 1}}`; `Guppy.Style.w("full")` produces `{:width, {:fraction, 1}}`; `Guppy.Style.top(-2)` produces `{:inset, :top, {:rem, -0.5}}`; `class="hidden invisible overflow-x-scroll cursor-pointer border-1 rounded-sm bg-red text-white shadow-md flex-col items-center text-xl font-bold grid-cols-3"` produces display, visibility, overflow, cursor, border-width, border-radius, color, shadow, flex/alignment, text/font, and grid tuple ops. Later ops win over earlier ones, and order is preserved through the bridge. In `~GUI` templates, `class` may be a string or a dynamic list of strings; `nil` and `false` list entries are ignored. Raw `style` values must be canonical style lists (for example `style={Guppy.Style.py(1)}` or `style={[Guppy.Style.py(1)]}`); use `class` for class-token strings.
